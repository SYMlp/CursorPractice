// Placeholder for migrated asset.ts mock content
// 资产监控页面的模拟数据
...
export const assetDetailData = {
  ...
  security: {
    ...
    securityMeasures: ['WAF防护', '定期漏洞扫描', '数据加密传输', '访问控制'],
    complianceStatus: '合规',
    certificateInfo: {
      hasSSL: true,
      issuer: 'DigiCert Inc',
...
// 添加从 assetData.ts 迁移过来的模拟数据生成函数
export const generateAssetChartData = (range: 'day' | 'week' | 'month') => {
  const baseData = {
  ...
  const generateRandomData = (base: number, range: number): number[] => {
    return Array(7)
      .fill(0)
      .map(() => Math.floor(base + Math.random() * range))
  }

  // 生成业务趋势数据 (确保有负值以模拟原始风格)
  const businessData = generateRandomData(5, 40)
  const randomIndex = Math.floor(Math.random() * businessData.length)
  businessData[randomIndex] = -Math.abs(businessData[randomIndex] * Math.random()) // 添加随机负值

  // 假设 extendedSecurityDistributionData 也在这个文件或已导入
  // 如果不在，需要从 React 的 assetData.ts 迁移过来或调整这里的逻辑
  const extendedSecurityDistributionData = [
