...
interface MenuItem {
  id: string;
  label: string;
  icon: React.ReactNode; // 使用 React.ReactNode 来容纳 SVG 组件
}

// 2. 定义 menuItems 数组
const menuItems: MenuItem[] = [
  { id: 'platform', label: '平台总览', icon: <OverviewIcon className="h-6 w-6" /> },
  { id: 'interface', label: '接口监控', icon: <InterfaceIcon className="h-6 w-6" /> },
  { id: 'asset', label: '资产监控', icon: <AssetIcon className="h-6 w-6" /> },
  { id: 'security', label: '数据资产监控', icon: <SecurityIcon className="h-6 w-6" /> },
];
    ...
    return (
      <div className="p-6">
        <Routes>
          ...
          <Route path="/notification-example" element={<NotificationServiceExample />} />
              ...
              <ul className="py-4">
                  ...
                  >
                    <Link
                      ...
                      className={`px-6 py-3 flex items-center space-x-3 cursor-pointer transition-colors ${
                        location.pathname === `/${item.id}` ? 'bg-blue-100 text-blue-800 border-r-4 border-blue-800' : 'hover:bg-gray-100'
                      }`}
                    >
                      {item.icon}
                ...
                >
                  <Link
                    ...
                    className={`px-6 py-3 flex items-center space-x-3 cursor-pointer transition-colors ${
                      location.pathname === '/notification-example' ? 'bg-blue-100 text-blue-800 border-r-4 border-blue-800' : 'hover:bg-gray-100'
                    }`}
                  >
